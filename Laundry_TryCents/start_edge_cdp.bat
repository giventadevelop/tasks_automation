...
set "TRYENTS_HOME=https://app.trycents.com/new-order/N3c4/home"
...
echo       Profile: C:\edge-cdp-laundry  ^(one-time TryCents login there^)
start "" "%EDGE_EXE%" ^
    --remote-debugging-port=%CDP_PORT% ^
    --remote-allow-origins=* ^
    --user-data-dir=C:\edge-cdp-laundry ^
    --no-first-run ^
...
echo   3. If Chrome or another app uses port 9222, quit it and re-run.
exit /b 1
