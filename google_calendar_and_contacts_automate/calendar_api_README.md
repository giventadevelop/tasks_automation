...
6. **PyInstaller / frozen `.exe` only:** Laundry and YouTube buttons run batch files under `scripts_batch_files/` in your real `tasks_automation` checkout (not inside the onefile bundle). Set `TASKS_AUTOMATION_ROOT` in the environment **or** in `calendar_api_properties.properties` (same key) to the repo root. If unset, the app tries `tasks_automation` next to the `.exe` and then common Windows checkout paths.

**Launchers** (from `scripts_batch_files/dashboard/` at repo root):
- `launch_daily_tasks_dashboard.bat` — runs `dist/calendar_automate.exe` when built (production).
- `launch_daily_tasks_dashboard_dev.bat` — always runs `google_calendar_and_contacts_automate.py` (latest Python source).
- `build_daily_tasks.bat` — rebuild the `.exe` after dashboard code changes.

Both launchers set `TASKS_AUTOMATION_ROOT` to the repo root automatically.
